...
def load_unik3d_model(
    ...
    if not pretrained:
        # A complete UniSHARP checkpoint already contains the encoder/decoder.
        # Construct locally: hubconf APIs vary and a Hub fallback would silently
        # download redundant pretrained weights even when pretrained=False.
        import json
        from unik3d.models import UniK3D  # type: ignore

        config_path = _get_ml_unisharp_root() / "UniK3D" / "configs" / "eval" / f"{backbone}.json"
